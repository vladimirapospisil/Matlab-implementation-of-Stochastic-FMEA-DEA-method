% compute FMEA with monte carlo noise sampling
clear all
...
for i=1:n_mc
    ...
    % compute RPN
    values(:,i) = O_noise(:,i).*S_noise(:,i).*D_noise(:,i);
end

mytime = toc; % stop to measure time

% save results for later use
results_fmea_mc.OSD = OSD; % problem values
results_fmea_mc.OSDn = OSDn; % normalized problem values
results_fmea_mc.sigma_OSD = sigma_OSD;
results_fmea_mc.n_mc = n_mc;
results_fmea_mc.O_noise = O_noise; % O with noise
results_fmea_mc.S_noise = S_noise; % S with noise
results_fmea_mc.D_noise = D_noise; % D with noise
results_fmea_mc.values = values; % results
results_fmea_mc.mytime = mytime;

% save file
save(['results/' problem_name '_fmea_mc.mat'],'results_fmea_mc')
